...
class PropertySyncTest extends TestCase
{
    ...
    public function testCleanUnit()
    {
        $unit = '# 5';

        $pSync = new PropertySync();

        $result = $pSync->cleanUnit($unit);

        $this->assertSame($result, '5', 'Remove Hashtag from unit');
    }

    public function testGetPropertyId()
    {
        $propertySync = new PropertySync();
        $result =  $propertySync->getPropertyId('23 Monmouth Street, Unit 1R, Somerville, MA');
