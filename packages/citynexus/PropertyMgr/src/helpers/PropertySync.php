...
class PropertySync
{
    public function address($data, $sync)
    {
    ...
    {
        try{
            $return = DB::select("SELECT * FROM standardize_address('tiger.pagc_lex', 'tiger.pagc_gaz', 'tiger.pagc_rules', '" . $address . "')")[0];
        }
    ...
    private function parsedAddress($row, $sync)
    {
    ...
    public function getPropertyId($address)
    {
        if(!is_array($address))
        {
            $addy = Address::firstOrCreate(['unparsed' => $address]);
